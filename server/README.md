...
- `BoardVersion` — snapshot historii (board_id, title, document, note, created_by, created_at)
- `BoardArchive` — stan archiwizacji (board_id — obecność wiersza = zarchiwizowana)
...
- `/boards` — GET (`?archived=true|false`, domyślnie aktywne), POST, GET/{id} (read), PUT/{id} (edit), DELETE/{id} (owner)
...
- `/boards/{id}/archive` · `/unarchive` — POST (owner): archiwizuje / przywraca tablicę (bez usuwania)
- `/boards/{id}/versions` — GET (read: lista), POST (edit: ręczny snapshot); zapis tablicy tworzy wersję automatycznie
